/* @pjs preload="http://localhost:8888/miniNYC.png,http://localhost:8888/logosign";*/
...
Map nyc;
var mapname = 'Kanye'; //CHANGETHIS CHANGE THIS <--- ctrl-c-ctrl-f-ctrl-v for others
...
if(pathArray.length > 0 && pathArray[0] != "songid"){
  ...
  logo = loadImage("http://localhost:8888/logosign");
...
//else $("#dialogWindow").css("position","absolute");
  ...
  YBASE = 30;
  XBASE = 0;
  ygrid = 5000;   //CHANGETHIS CHANGE THIS
  xgrid = 5000;	  //CHANGETHIS CHANGE THIS
togglePlayer();
...
void setUpLocations(){
	$.getJSON('http://localhost:8888/loc/browse?city='+mapname+'&hasLoc=8', function(results){      
...
//	2000 x 1422
	...
	*/
	void prep(){
		...
		widths = new Array(1250,1250,1250,1250); //CHANGETHIS CHANGE THIS
		heights = new Array(1250,1250,1250,1250); //CHANGETHIS CHANGE THIS
		allX = 5000; allY = 5000;
		setMins();
	}
			...
			if(iconready){			
				if(iconset){
					$.ajax({
						...
						data: {_id:_id, x:iconx+'', y:icony+'',city:mapname},
				...
				if(_id && _id.length > 6){
					curicon = requestImage("http://localhost:8888/loc/getTypeIconID?city="+mapname+"&_id="+_id);
					iconrequested = true;
				}
			}	
		}
