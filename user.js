...
function writeError(res, value) {
  ...
  var returnable = {response: "error"};
  if(value != null)
    returnable["value"] = value;
		...
		usersCollection.findOne({_id:userID},function(err,user){
			if(err){
				...
				var returned = {'success':true,'exists':true,'streetCredit':sc,
					...
					noWelcome: user.noWelcome, noToolbox: user.noToolbox};
				if(user.scope) returned['scope'] = true;
...
//function post adding publish_action scopes
function addScope(fbid,accessToken,callback){
	console.log('GOT TO THE ADD SCOPE FUNCTION');
	usersCollection.findOne({'fbid':fbid}, function(err, user){
		if(err){
			console.log(err);
			callback("error finding user in database");
		}
		else if(user){
			usersCollection.update({'_id':user._id},{'$set':{accessToken:accessToken, scope:'publish'}},
			function(err,fin){
				if(err){
					callback("updating accesstoken error");
				}
				else{
		...
		else callback("user not ready for superscope");
	});
}
...
function createUser(fbData, callback, session) {
	usersCollection.findOne({'fbid':fbData.fbid}, function(err, user){
		if(err){
			...
			usersCollection.update({'_id':user._id},{'$set':{accessToken:fbData.accessToken}},
			function(err,fin){
				if(err){
					callback("updating accesstoken error");
				}
...
function fbRecommend(req, res){
	console.log('\n\nGOT TO USERS FUNCTION\n');
	if(req.user && req.body){
		console.log('\nREQ DOT USER AND BODY EXIST\n');
		var userID = new ObjectID(req['user']);
		usersCollection.findOne({_id:userID},function(err,user){
			if(err){
				writeError(res,'no such user!');
				...
				var postdata = {};
				var posturl = 'http://localhost:8888/l/'+req.body._id;
				if(req.body.RID) posturl += "/" + req.body.RID;
				if(req.body.type == 'artist' || req.body.type == 'biggie' || req.body.type == 'rappersignificant') postdata['song'] = posturl;
				else postdata['video'] = posturl;
				postdata['fb:explicitly_shared'] = true;
				//send request to facebook
				graph.setAccessToken(user.accessToken)
				.post("/me/rapcities:recommend", postdata,function(err,data){
					console.log('\n\nRESPONSE FROM FACEBOOK:\n');
					if(err){ console.log(err); writeError(res,err);}
					else{ console.log(data); writeSuccess(res);}
				});
			}	
		});
	}
...
//disables toolbox message
function noToolbox(req,res){
	var userID = new ObjectID(req['user']);
	usersCollection.update(
		{'_id': userID},
		{'$set': {noToolbox:true}}, 
